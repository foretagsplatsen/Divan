...
requests so we detect if we are running in Mono and then we fall back on using a regular GET.
...
4. If you are on Mono "CLI style" then the configure/Makefile should work as expected, it is "vanilla generated" from Monodevelop. Then you can also run unit tests, but note that these rely on a couchdb running locally:
